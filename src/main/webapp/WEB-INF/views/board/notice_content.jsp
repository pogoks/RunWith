			...
			<div class="container">
				<div class="row">
					...
					<a href="#" class="list-group-item active notice-list-top"
						style="margin-top: 20px;"> <span class="main-notice-title">공지사항</span>
					</a>


					<div class="container my-1">
						<form action="<c:url value='/notice/noticeDelete'/>" method="post"
							name="noticeDeleteForm">
							<div class="row">
								<!-- 공지사항 글 상세보기  -->
								<div class="notice_content">
									<div class="notice_content_up" style="margin-left: 30px;">
										<div class="notice_title" scope="col"
											...
											<c:if test="${user.commonCODE == 'ADM002'}">
												<button type="button" id="btn-notice-delete"
													class="btn mb-2"
													style="display: inline-block; float: right; margin-right: 50px;">삭제</button>
											</c:if>
										</div>
										...
										<div style="margin-top: 30px;">
											<div class="notice_writer" style="display: inline-block;">
												<img
													src="<c:url value='/user/userProfileGet?userNO=${noticeWriterNo}'/>"
												...
												<span> <span
													style="display: inline-block; font-size: 14px; font-weight: bold; font-family: sans-serif; margin-left: 15px;">&nbsp;
														${notice.noticeWriter} &nbsp;</span> <c:if
														test="${notice.noticeWriter eq user.userID }">
								...
								<!-- 글 내용 -->
								<div class="notice_content_down"
									style="min-height: 300px; margin-top: 50px; margin-left: 30px; font-size: 15px; margin-bottom: 30px;">
									${fn:replace(notice.noticeContent, cn, br) }</div>
							</div>
							<hr class="borderline" />

							<c:if test="${notice.noticeWriter eq user.userID }">
								<button type="button" id="btn-notice-modify"
									class="btn btn-info pull-right"
									onclick="location.href='<c:url value="/notice/noticeModify?noticeNo=${notice.noticeNo}"/>'"
									style="margin-left: 10px;">수정하기</button>
							</c:if>
							<button type="button" id="btn-notice-list"
								class="btn btn-primary mb-2 pull-right"
								onclick="location.href='<c:url value="/notice/noticeList"/>'"
								style="margin-left: 10px;">목록</button>
