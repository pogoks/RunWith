...
<body>
	...
	<div id="navbar-wrapper">
		...
		<input type="hidden" id="hidden-room-name" value="${roomName}" />
			...
			<section style="float: left; width: 100%; height: 90%; background: white; border: 1px solid #E9E9E9;">
				...
				<div style="padding: 5px; border-bottom: 2px solid #E9E9E9; background: #FCFCFC;">
					<a href="#" class="list-group-item active notice-list-top" style="width: 100%; border: 0; background: #FCFCFC; cursor: default;"> 
						...
						<span class="main-board-title" style="color: #4965FE; font-size: 16px; font-family: sans-serif;"><span class="main-board-title" style="color: #390001; font-size: 16px; font-family: sans-serif;">Channel : </span>${roomName}&nbsp;&nbsp;
							<span class="pull-right" style="line-height: 2.0em; margin-right: 40px; font-size: 11px; font-weight: 500;">현재 참여자 : <span id="onlineCount" style="color: #EA00AA; font-weight: 500; font-size: 12px;"></span> 명</span>
						</span>
...
<script>
	...
	function remove() {
		...
		var onlineCount = $('#onlineCount').text();
		var roomNO = $('#hidden-room-no').val();
		var roomName = $('#hidden-room-name').val();
		
		if(onlineCount == 1) {
			
			$.ajax({
				type: 'POST',
				url: '<c:url value="/chat/chatDeleteRoom" />',
				async: false,
