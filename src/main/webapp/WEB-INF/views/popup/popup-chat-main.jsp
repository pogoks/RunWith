			...
			<section style="float: left; width: 100%; height: 90%; background: white; border: 1px solid #E9E9E9;">
				...
				<div style="padding: 5px; border-bottom: 2px solid #E9E9E9; background: #FCFCFC;">
					<a href="#" class="list-group-item active notice-list-top" style="width: 100%; border: 0; background: #FCFCFC; cursor: default;"> 
						...
						<span id="btn-chat-refresh" class="main-board-title pull-right" style="color: #808080; font-size: 12px; font-family: sans-serif; margin-right: 30px; font-weight: 500; cursor: pointer;"><i class="fa fa-refresh" aria-hidden="true" style="line-height: 1.8em;"></i> 새로고침</span>
					</a>
				</div>
...
<script>
	...
	function createChatingRoom(res){
		if(res != null){
			...
			res.forEach(function(d, idx){
				...
				var idx = idx;
				strAdd += `<tr>
						    	<td data-value="` + idx + `" style="width: 10%;" onclick='goRoom(\"` + roomNO + `\", \"` + roomName + `\")'>` + (idx + 1) + `</td>
						    	<td data-value="` + idx + `" style="width: 70%;" onclick='goRoom(\"` + roomNO + `\", \"` + roomName + `\")'>` + roomName + `</td>
						    	<td data-value="` + idx + `" style="width: 20%;" onclick='goRoom(\"` + roomNO + `\", \"` + roomName + `\")'>` + roomOwner + `</td>
						  </tr>`;
			});
	...
	$(function() {
		...
		// 새로고침 버튼 클릭 시
		$('#btn-chat-refresh').click(function() {
			getRoom();
		});
		
		
		$('#chatRoomList').on('click', 'td', function(e) {
